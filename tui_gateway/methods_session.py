...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        tdefs = get_tool_definitions(enabled_toolsets=["hermes-cli", "skills"], quiet_mode=True, skip_tool_search_assembly=True)
        for t in tdefs:
            name = t["function"]["name"]
            ts = get_toolset_for_tool(name) or "other"
            tools_dict.setdefault(ts, []).append(name)
    except Exception:
        pass
    return _ok(rid, {
        ...
        "info": {"model": override.get("model") if override else _resolve_model(),
                 **({"provider": override["provider"]} if override.get("provider") else {}),
                 "tools": tools_dict, "skills": get_available_skills(), "cwd": cwd, "branch": git_probe.branch(cwd),
