...
class CLIStreamMixin:
    ...
    def _on_tool_gen_start(self, tool_name: str) -> None:
        ...
        if getattr(self, "_stream_box_opened", False):
            ...
            self._stream_box_opened = False
        self._close_reasoning_box()
        # Suppress duplicate consecutive "preparing delegate_task..." on busy poll loops
        if tool_name == "delegate_task" and getattr(self, "_last_prepared_tool", None) == "delegate_task":
            return
        self._last_prepared_tool = tool_name
